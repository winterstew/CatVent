...
# Initialize VCNL4200
proxlong = adafruit_vcnl4200.Adafruit_VCNL4200(i2c)
# Initialize SHT41
...
proximitylong_feed = 'cat-vent.proximitylong'
illuminance_feed = 'cat-vent.illuminance'
illuminancelong_feed = 'cat-vent.illuminancelong'
...
while True:
    ...
    if (time.monotonic() - lastvocindex) >= vocindex_rate:    
        ...
        lastvocindex = time.monotonic()
    print(f"Proximity is: {prox.proximity}")
    print(f"Ambient Illuminance is: {prox.lux}")
    print(f"Proximity Long is: {proxlong.proximity}")
    print(f"Ambient Illuminance Long is: {proxlong.lux}")
    ...
    if (time.monotonic() - lastpublish) >= publish_rate:
        ...
        io.publish(proximitylong_feed, proxlong.proximity)
        io.publish(illuminancelong_feed, prox.lux)
