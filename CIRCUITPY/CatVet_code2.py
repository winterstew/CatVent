...
rawgas_feed = ('cat-vent.rawgas-1', 'cat-vent.rawgas-2')
...
while True:
    ...
    for box in range(boxes):
        if sht[box]:
            ...
            if sgp[box]:
                if (time.monotonic() - lastvocindex) >= vocindex_rate:    
                    raw_gas[box] = sgp[box].raw
                    if debug > 2: print("Measurement in %1d: %d" % (box+1,raw_gas[box]))
        ...
        if (voc_index[0] < vocindex_low and voc_index[1] < vocindex_low 
            and not relay_open and (time.monotonic() - ontime) > ondelay):
            open_relay(setpin, unsetpin)
            io.publish(onoff_feed, "Off")
    if debug > 2:
    ...
    if (time.monotonic() - lastdisplay) >= display_rate:
        ...
        if (display_counter == 0):
            ...
            boxdisplay = (boxdisplay + 1) % boxes
        display.show()
        lastdisplay = time.monotonic()
    if (time.monotonic() - lastpublish) >= publish_rate:
        io.publish(temperature_feed[boxpublish], temperature[boxpublish])
        io.publish(humidity_feed[boxpublish], humidity[boxpublish])
        io.publish(rawgas_feed[boxpublish], raw_gas[boxpublish])
        io.publish(vocindex_feed[boxpublish], voc_index[boxpublish])
        io.publish(proximity_feed[boxpublish], proximity[boxpublish])
        io.publish(illuminance_feed[boxpublish], illuminance[boxpublish])
